...
- `invoke` 成功返回应遵循 `llm_webop_v2` 回执字段：
  - `ok=true`
  - `contractVersion="llm_webop_v2"`
  - `action.targetId` / `action.fn`
  - `page.urlBefore` / `page.urlAfter`
  - `recovery.retryable` / `recovery.suggestedNext`
  - 业务数据放在同层扩展字段中（如 `files`、`selectedSql` 等）
- 需要 MAIN world 执行时，统一走 SDK `invokeInMainWorld`（`playwrong.mainworld.invoke`），不要新增动作特化通道。
- 对导航型动作（提交、跳转）必须保证可重试，不依赖单次句柄。
- 必须提供至少一个无副作用 `debug*` page function（用于回传当前可识别节点/表单与动作可用性）。
